...
if (!$user || (strtolower($user['school']) !== 'samifrasheri' && strtolower($user['isadmin']) !== 'true')) {
    echo "Access denied.";
    exit();
}
...
$studentsStmt = $conn->prepare("SELECT id, name, surname FROM users WHERE school = 'samifrasheri' AND isstudent = 'true'");
...
$query = "SELECT g.*, u.name, u.surname FROM student_grades g JOIN users u ON g.student_id = u.id WHERE u.school = 'samifrasheri'";
$params = [];
...
$commentsStmt = $conn->prepare("SELECT c.comment, c.timestamp, u.name, u.surname, CASE WHEN u.isadmin = 'true' THEN 'Admin' ELSE 'Teacher' END AS role FROM comments c JOIN users u ON c.user_id = u.id WHERE u.school = 'samifrasheri' ORDER BY c.timestamp DESC");
...
<body>
    <div class="navbar">
        <div><strong>Sami Frasheri</strong></div>
        <div>
            <?php
    ...
    <div class="container">
        ...
        <div class="button-bar">
            <?php if ($isTeacher || $isAdmin): ?>
                <a href="samifrasheri_addGrade.php">Add Grade</a>
                <a href="samifrasheri_editGrades.php">Edit Grades</a>
                <a href="samifrasheri_addComment.php">Add Comment</a>
            <?php endif; ?>
        </div>
